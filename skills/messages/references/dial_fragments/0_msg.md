...
- No verified hook for this contact → fall back to segment-generic personalization: the industry
  plus a registry-sourced fact. Do not stretch.
- Every factual sentence cites a `fact_id` in the claims_manifest. A sentence you cannot tie to a
  verified `fact_id` gets cut, not hedged.
- Manifest classes allowed: `verified` only. Nothing about the prospect is claimed beyond the
  dossier; nothing about US is claimed beyond `claims.assertable`.
- Export is blocked until 100% of manifest claims are CONFIRMED — draft so that check can pass.
