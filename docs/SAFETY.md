...
**3. Claims policy by dial.** The Caution Dial governs how a draft's claims are handled, and it is a gate the skill applies row by row. At dial 0–1, a Messages row whose `claims` include anything marked FALSE or UNVERIFIABLE gets `STATUS=blocked` and is never copied to the Queue tab. At dial 2–3 the same check runs but the row goes ready with the verdict shown as a visible flag rather than blocked. Three invariants hold at every dial — no fabricated proper nouns; certification and compliance claims are verified-only; and an inferred reporting-structure claim (who reports to whom / who runs a function, sourced only from title inference) never appears in message copy — and a claim marked FALSE is always corrected. Those three are corrected or cut at every dial, never merely flagged. The block is a rule stated in the skill, applied to the fact-check verdicts, so it is decided by the recorded evidence, not by a draft that argues it is fine.
